...
def check_if_admin_or_student():
    ...
    if attempt_counter == attempt_lockout: # checking the number of times the user has clicked the button for sign in lockout to occur
        ...
        st_uname_hash, st_pdw_hash, st_id = classes_sms.Student.student_login_details() #get the values of student login details
        ...
        for st_d_u_keys, st_list_username in st_uname_hash.items():
            for username_item in st_list_username:
                
                if bcrypt.checkpw(a_s_username_bytes, username_item): # checking if studnet username was entered
                    
                    for st_d_pdw_keys, st_list_password in st_pdw_hash.items():
                        for password_item in st_list_password:
                    
                            if bcrypt.checkpw(a_s_password_bytes, password_item): # checking if student password was entered
                                ...
                                studentSMS.start(st_id, student_name= a_s_username) 
